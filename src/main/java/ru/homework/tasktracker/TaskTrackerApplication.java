	...
	public CommandLineRunner addGreeting() {
		return args -> {
			...
			String userRules = "Команды сервиса \"user\":\n" +
					...
					"2. \"viewmt\" - поиск пользователя с максимальным количеством задач.\n" +
					"    Можно отфильтровать задачи по статусу и по времени, для это надо после id ввести фильтр: \n " +
					"   \"status=(CREATED/RUN/COMPLETED)\"&\"date=from,to\"\n" +
					"    Примеры команд: \"viewt 1\", \"viewt 1 status=RUN&date=20.07.2022,25.07.2022\"\n" +
					"3. \"viewt\" - просмотр задач пользователя. После команды надо ввести id пользователя, чьи задачи ты хочешь посмотреть\n" +
					"    Можно отфильтровать задачи по статусу и по времени, для это надо после id ввести фильтр: \n " +
					"   \"status=(CREATED/RUN/COMPLETED)\"&\"date=from,to\"\n" +
					"    Примеры команд: \"viewt 1\", \"viewt 1 status=RUN&date=20.07.2022,25.07.2022\"\n" +
					"4. \"create\" - создание пользователя. После команды надо ввести перечень аргументов, расположенных в строгом порядке: имя\n" +
					"    Пример команды: \"create Вася\"\n" +
					"5. \"edit\" - изменение пользователя. После команды надо ввести id пользователя, которого собираешься изменять,\n" +
					"    а затем перечень аргументов, расположенных в строгом порядке: имя\n" +
					"    Примеры команд: \"edit 1 Петя\"\n" +
					"6. \"delete\" - удаление пользователя. После команды надо ввести id пользователя, которого собираешься удалить\n";
			String taskRules = "Команды сервиса \"task\":\n" +
					...
					"    Примеры команд: \"view\", \"view 1\", \"view status=RUN\"\n" +
